...
static void Init(void)
{
  ...
  //SystemClock_Config();
    ...
    PRINTF("Last Reset Cause SRC(SRSR)=0x%x\r\n",SRC->SRSR);
    SRC->SRSR = SRC->SRSR;
    PRINTF("Last Reset Cause SRC(SRSR)=0x%x\r\n",SRC->SRSR);
//    BOARD_InitModuleClock();
} /*** end of Init ***/

/*Deinit PLL clock, LPUART/Debug Uart, ENET etc*/
void Deinit()
{
  //must do deinit clock and uart
  ...
  #endif
#endif
    //change PLL will influence flexspi, so don't adjust them
