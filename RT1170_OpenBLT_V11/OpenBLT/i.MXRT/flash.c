...
#define FLASH_INVALID_SECTOR            (0xFFFFFFFF)
/** \brief Value for an invalid flash address. */
#define FLASH_INVALID_ADDRESS           (0xffffffff)
/** \brief Standard size of a flash block for writing. */
#define FLASH_WRITE_BLOCK_SIZE          256 //(512)
...
#define BOOT_FLASH_VECTOR_TABLE_CS_OFFSET    (0x1C8)//´ý¶¨
#endif
...
#define BOOT_FLASH_TEST 0//set 1 to bypass all real flash operation and return true.

#define BOOT_FLASH_CUSTOM_LAYOUT_ENABLE 1 //use customer flash layout, since QSPI flash size is huge
...
typedef struct
{
  ...
  blt_int32u  sector_num;                         /**< sector number                    */
} tFlashSector;
...
static blt_bool  FlashEraseSectors(blt_int32u first_sector, blt_int32u last_sector);
static blt_int32u FlashGetSector(blt_addr address);
...
#define FLASH_SIZE_BYTE  (0x1000000) //IS25WP128 128Mb. 16MB
#define FLASH_SECTOR_NUM (FLASH_SIZE_BYTE/SECTOR_SIZE)
#define APP_FLASH_SECTOR_START_SECTOR 16 //reserve 64K for bootloader 
#define APP_FLASH_SECTOR_NUM (FLASH_SECTOR_NUM-APP_FLASH_SECTOR_START_SECTOR) 

//4KB sector size for qspi flash, use code to generate this table
static tFlashSector flashLayout[APP_FLASH_SECTOR_NUM] = {0};
...
void AllocFlashLayout()
{
  uint32_t i;
  
  for(i=0;i<APP_FLASH_SECTOR_NUM;i++)
  {
    flashLayout[i].sector_start = EXAMPLE_FLEXSPI_AMBA_BASE + (APP_FLASH_SECTOR_START_SECTOR+i)*SECTOR_SIZE;
    flashLayout[i].sector_size = SECTOR_SIZE;
    flashLayout[i].sector_num = APP_FLASH_SECTOR_START_SECTOR + i;
  }
}
/************************************************************************************//**
...
void FlashInit(void)
{
  static uint8_t FlashIsInited = false;
...
{
  blt_int32u first_sector;
  blt_int32u last_sector;
...
#if (BOOT_FLASH_TEST)
    PRINTF("erase addr 0x%x, len = 0x%x\r\n",addr, len);
#endif
...
#if (BOOT_FLASH_TEST)
  PRINTF("Signature_checksum 0x%x\r\n",signature_checksum);
  return true;
#endif
...
#if (BOOT_FLASH_TEST)
  PRINTF("read checksum from 0x%x, =0x%x\r\n",flashLayout[0].sector_start,signature_checksum);
  return true;
#endif
...
static blt_bool FlashWriteBlock(tFlashBlockInfo *block)
{
  ...
  PRINTF("%s@%d\r\n", __func__, __LINE__);
  PRINTF("write block at addr 0x%x\r\n",block->base_addr);
#if (BOOT_FLASH_TEST)
  return true;
#endif
...
//    prog_addr -= EXAMPLE_FLEXSPI_AMBA_BASE;
//    prog_data = *(volatile blt_int32u *)(&block->data[word_cnt * sizeof(blt_int32u)]);
//    /* keep the watchdog happy */
//    CopService();
//    /* program the word */
//    //if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, prog_addr, prog_data) != HAL_OK)
//    status = flexspi_nor_flash_program(EXAMPLE_FLEXSPI, prog_addr, (void *)&prog_data, sizeof(blt_int32u));
//    if (status != kStatus_Success)
//    {
//      result = BLT_FALSE;
//      break;
...
//  }
    ...
    status = flexspi_nor_flash_page_program(EXAMPLE_FLEXSPI, block->base_addr-EXAMPLE_FLEXSPI_AMBA_BASE, (void *)block->data);
    if (status != kStatus_Success)
    {
  ...
  if(result == BLT_TRUE)
  {
    DCACHE_InvalidateByRange(block->base_addr, FLASH_WRITE_BLOCK_SIZE);
    
    //memcpy(s_nor_read_buffer, (void *)(EXAMPLE_FLEXSPI_AMBA_BASE + EXAMPLE_SECTOR * SECTOR_SIZE),
    //       sizeof(s_nor_read_buffer));
    
    if (memcmp((void*)(block->base_addr),  (void *)block->data, FLASH_WRITE_BLOCK_SIZE) != 0)
    {
...
#if (BOOT_FLASH_TEST)
    PRINTF("erase sector from sec %d-%d\r\n",first_sector, last_sector);
    return true;
#endif
...
//    HAL_FLASH_Unlock();
    ...
    for (sectorIdx=first_sector; sectorIdx<= last_sector; sectorIdx++)
    {
      ...
      status = flexspi_nor_flash_erase_sector(EXAMPLE_FLEXSPI, sectorIdx * SECTOR_SIZE);
      if (status != kStatus_Success)
      {
      ...
      DCACHE_InvalidateByRange(EXAMPLE_FLEXSPI_AMBA_BASE + sectorIdx * SECTOR_SIZE, SECTOR_SIZE);
...
static blt_int32u FlashGetSector(blt_addr address)
{
  blt_int32u result = FLASH_INVALID_SECTOR;
  blt_int32u sectorIdx;
